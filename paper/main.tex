...
Several ingredients of this framework exist individually, and the contribution
should not be read as larger than it is. That
...
per-configuration index, is established in~\cite{wong2024sensorobs}. That Jacobian
conditioning governs the accuracy of the reconstruction and can be used to select
postures is established in~\cite{lu2023configopt}. Motor current has been used in
place of joint torque sensing~\cite{wahrburg2018motorcurrent}, and the
point-contact reduction is already the practical default on low-cost
arms~\cite{yen2019virtual}.

None of them reports how much of an applied force the six-dimensional inversion
discards, in which directions, and how often across a workspace. Nor that this
quantity is not scale-free, nor which part of it is. Nor that the loss is silent
under a conventional validation. And none addresses the velocity-actuated regime.
The $50.5\,\%$ figure is likewise not a general result: it belongs to this arm,
this workspace, and one choice of the length that renders the wrench norm
homogeneous. It carries weight because $\Pff$ explains and predicts it, and
because the same construction can be evaluated for any other arm before one is
built.
