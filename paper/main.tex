...
structure of the error: the loss is a linear function of the alignment with the
least observable direction, exactly so at each posture and at correlation
$0.9993$ over $12\,000$ push directions pooled across six.
The magnitude of that loss is shown to depend on the characteristic length that
makes the wrench norm dimensionally homogeneous, a choice the unweighted
pseudoinverse makes silently; the directions lost do not. Over $2197$
configurations of a 4-DoF arm the six-dimensional inversion discards $50.5\,\%$ of
an applied unit force on average under the unweighted convention in standard use,
$38.0\,\%$ under the arm's own scale, and up to $99.6\,\%$ under either, while
still correlating with ground truth at $0.997$, so a validation that looks
convincing can hide a large gain error. The point-contact inversion admits no such
choice of metric.
Rejecting postures below $\sigma_{\min}(\bm J_{\!v})=0.06$ halves the
ninetieth-percentile error of the point-contact estimator at the cost of
$69\,\%$ of the workspace, and leaves the
...
cost. All results are obtained in simulation, against a second and independently
written dynamic implementation that agrees with the model to $10^{-10}$ and that
serves as the plant; agreement at that level establishes implementation
...
with the least observable direction, exactly so at each posture and at correlation
$0.9993$ pooled over $12\,000$ directions, and over $2197$ configurations it
discards $50.5\,\%$ of an applied unit force on average and up to $99.6\,\%$. Two
of those three statements are properties of the arm alone. The third is not: the
minimum-norm criterion adds newtons to newton-meters and therefore presupposes a
length, so the magnitude of the loss is reported here against that length, falling
to $38.0\,\%$ at the arm's own scale, while the directions in which the loss
occurs and the set of exactly recovered forces are invariant to it. That the
point-contact inversion requires no such choice is an argument in its favor
independent of accuracy. The failure of the six-dimensional inversion is also
silent, coexisting
...
workspace; this leaves numerical conditioning as the binding limitation and makes
$\sigma_{\min}(\Jv)$ the quantity to monitor. Against an independently written
dynamic implementation the correction improves force RMSE by a factor of fourteen.
...
$69.6\,\%$, and gating on $\sigma_{\min}(\Jv)$ halves its tail, at the cost of
$69\,\%$ of the workspace, while leaving the
six-dimensional error untouched, separating amplified noise from structural
misattribution. Finally, because the actuator saturates below the force range of
manual guidance, physical compliance requires no control effort and the synthetic
loop need only cover fine guidance and the return to task.
