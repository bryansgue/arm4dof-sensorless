...
workspace; the demonstration that the loss is silent under a conventional
validation; and the velocity-actuated regime, which that literature does not
address.
...
\label{sec:realtime}
Real-time rates depend on how the forward dynamics is expressed symbolically.
...
bottleneck. Rebuilding the identical model on the scalar type turns that solve into
symbolic Gaussian elimination and yields the timings of Table~\ref{tab:timing},
with the controller unchanged: over $250$ closed-loop steps the two builds agree to
$2.2\times10^{-15}\,\mathrm{N}\cdot\mathrm{m}$ in commanded torque. We report this
because the failure mode is easy to reach and presents as a slow solver rather
than as a modeling error.
