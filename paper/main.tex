...
\title{Sensorless Contact-Force Estimation on Low-DoF Manipulators}
...
\markboth
{J. Varela-Ald\'as \headeretal:~Sensorless Contact-Force Estimation on Low-DoF Arms}
{J. Varela-Ald\'as \headeretal:~Sensorless Contact-Force Estimation on Low-DoF Arms}
...
force/torque sensor, whose price exceeds the arm's, and with servos
commanded in velocity, not torque. That a residual observer cannot resolve
...
directions, and at which postures is not. This paper quantifies it for a
four-joint arm. The force block of the wrench projector predicts which
directions are lost, and over $2197$ configurations the six-dimensional inversion
discards $50.5\,\%$ of an applied unit force on average, returning it as a
spurious moment while still correlating with ground truth at $0.997$: a
validation that looks convincing can hide a large gain error. The magnitude
depends on the characteristic length that makes the wrench norm dimensionally
homogeneous, a choice the unweighted pseudoinverse makes silently; the directions
lost do not, and the point-contact inversion admits no such choice.
Velocity-level actuation, usually regarded as an obstacle, instead confines the
dynamic model to the estimator: the residual is invariant to the inner servo
loop, whose gain need not be identified, and a kinematic controller reproduces
the compliant behavior at one thirtieth of the cost. All results are obtained
in simulation, against an independently written dynamic implementation that
establishes correctness, not physical realism. The hardware experiment that would
test the directional prediction, the conditioning law, and torque inferred from
motor current is specified, not run.
\end{abstract}
...
\begin{proof}
By \eqref{eq:sixinv}, $\hat{\bm F}_6=\Pobs\bm F$, so with $\bm m=\bm 0$ its force
block is $\Pff\bm f$. Since $\Pobs$ is an orthogonal projector,
$\bm 0\preceq\Pff\preceq\bm I$ and hence $\Pff^2\preceq\Pff$. Idempotence of
$\Pobs$ gives $\norm{\hat{\bm F}_6}^2=\bm F^{\!\top}\Pobs\bm F=
\bm f^{\!\top}\Pff\bm f$, so the squared norm of the moment block is
$\bm f^{\!\top}(\Pff-\Pff^2)\bm f\ge0$, strictly positive unless $\bm f$ lies in
an eigenspace of $\Pff$ with eigenvalue $0$ or $1$.
\end{proof}
...
Servo loop gain $k_v=4$. The push is held for six admittance time constants and
the commanded column matches the analytic equilibrium
$\norm{\bm f}/(D_{\mathrm{trans}}k)$ to $0.3\,\%$. Every solve succeeded in every
row.}
\label{tab:regimes}
\centering
\footnotesize
...
At this loop gain the two indicators change together. Below the transition the arm follows the
...
actuator is saturated for $97$ to $100\,\%$ of the push. That the factor below
the transition exceeds unity is itself the mechanism of this section at a smaller
scale: the loop of this experiment is soft, so the arm deflects physically under
the push in addition to following the displaced reference. Both contributions are
linear in $\norm{\bm f}$ while the actuator is unsaturated, which is why their
sum is a constant multiple of the command rather than a growing one, and it is
the departure from that constant that marks the transition.

Both figures belong to this loop gain, and separating the regime from the tuning
requires varying it. At $k_v=20$ and $k_v=100$, a range of $25$, the ratio below
the transition falls to $0.97$--$1.22$, the stiffer loop suppressing the physical
deflection; above the transition it stays at $2.26$--$2.80$, and the transition
itself stays between $6$ and $8$~N at every gain. The regime boundary is a
property of the actuator bound, not of the servo tuning.

The saturation column does not survive that variation, and it is the indicator
rather than the conclusion that fails. The flag records whether the unclipped
demand $k_v(\bm u-\dot{\bm q})$ exceeds $\tau_{\max}$, so at $k_v\ge20$ the
starting transient alone trips it and the column reads $100\,\%$ in every row, at
$1$~N included. The displacement ratio separates the two regimes at every gain
tested; the saturation flag does so only at the soft gain of
Table~\ref{tab:regimes}.

The transition falls
...
with $k_v=20$ and torque saturation.}
...
\texttt{NAN\_DETECTED} at every solve; the failure belongs to the proportional
loop model \eqref{eq:servoloop}, not to velocity-level formulations in general,
see text. Solve times are given for two
...
solves between twenty-six and twenty-eight times faster \emph{in this regulation
test}; Section~\ref{sec:res-model} measures the same ratio over the complete
compliant loop, where it is thirty-one. We quote the ratio
...
A hand-guiding scenario is simulated at $100$~Hz on the plant
\eqref{eq:velplant} with $k_v=4$. The arm holds a task pose, an
...
model-free controller (V0). The estimator is identical in both. Yield is the
maximum end-effector displacement from the task pose during the push, return the
displacement remaining after release.}
\label{tab:v0loop}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
...
\midrule
in plane & T  & $0.0081$ & $63.7$ & $9.18$ & $2.14$\\
         & V0 & $0.0137$ & $50.2$ & $\bm{6.60}$ & $\bm{0.069}$\\
\midrule
normal   & T  & $0.0069$ & $54.5$ & $5.21$ & $2.15$\\
         & V0 & $0.0059$ & $44.1$ & $\bm{2.14}$ & $\bm{0.068}$\\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:v0loop} reports the outcome. Force estimation remains accurate to
between $0.10$ and $0.22\,\%$ of the applied push under both controllers. The
small difference between the two columns is not the estimator, which is identical,
but the trajectory along which it is evaluated, since the residual depends on the
finite-differenced acceleration and the two controllers do not follow the same
path. The compliant behavior is equivalent: the arm yields to $0.79$--$0.81$ of
the model-based displacement and returns closer to the task pose on release, in
both push directions. The computational cost falls by a factor of thirty-one,
against the twenty-six to twenty-eight of the regulation test of
Table~\ref{tab:formulations}; the two differ because the task and the loop gain
differ, not because either measurement is uncertain, and the quantity that
transports is that the saving is more than an order of magnitude in both.
Carrying the inertia matrix and the bias term in the controller therefore brings
no benefit on this platform, while carrying them in the estimator is unavoidable.
...
joint with random sign, which enters as a torque offset and is therefore the
class of error Section~\ref{sec:res-sens} finds dominant, and twelve-bit encoder
quantization. The stiction magnitude in
...
\emph{Scope of the model-free controller.} The equivalence between V0 and T is
established in the quasi-static regime of hand guiding on a four-joint arm, where
joint velocities stay well below the servo limit and the inertial term is small
against the actuator authority. It is not claimed at speed, under a large
payload, or on a mechanism where that ratio is different; there the kinematic
limit $\dot{\bm q}=\bm u$ ceases to approximate \eqref{eq:velplant}.
...
a model-based one throughout the compliant loop at one thirtieth of the cost.
