...
convention. The minimum-norm criterion sums newtons and newton-metres, so it
...
newton-metres. Adding their squares presupposes a length. The unweighted
pseudoinverse of $\Jfull$ expressed in SI units, which is what the standard
residual observer computes, therefore fixes that length at one metre without
saying so. Equivalently, the top three rows of $\Jfull$ carry units of metres and
the bottom three are dimensionless, so $\mathrm{col}\,\Jfull$, and with it
$\Pobs$, changes if lengths are expressed in millimetres instead.
...
order of magnitude below the metre that the unweighted convention imposes.
...
newton-metres were scaled against newtons.
...
between $6$ and $8$~N, which is where it should: the static force this posture can
...
that halves the tail accepts only $31.4\,\%$ of the workspace, so an arm
restricted to it forfeits two thirds of its reachable postures; at
...
minimum-norm criterion adds newtons to newton-metres and therefore presupposes a
...
rejecting two thirds of the workspace, while leaving the
six-dimensional error untouched, separating amplified noise from structural
...
(M'22--SM'24, IEEE) is an Industrial Engineer from the Universidad T\'ecnica de
