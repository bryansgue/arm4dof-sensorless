...
\begin{keywords}
Compliant control, dual quaternions, force estimation, low-cost robotics, model
...
\subsection{Dual quaternion representation and predictive control}
Dual quaternions provide a singularity-free representation of $\SEthree$ that
keeps translation and rotation in one algebraic object, and have been used for
kinematic control of manipulators~\cite{figueredo2013dq} and packaged for general
robot modeling~\cite{adorno2021dqrobotics}. The logarithmic map to $\sethree$
couples the two components through the left Jacobian of
$\mathrm{SO}(3)$~\cite{sola2018microlie}. We adopt this representation for the
tracking cost and, in Section~\ref{sec:res-ablation}, report an ablation isolating
what the coupling contributes on an under-actuated arm, where the reference is
generically unreachable. Real-time nonlinear predictive control is enabled by the
real-time iteration scheme~\cite{diehl2005rti} and by embedded solver
...
displaced dual quaternion reference, and the predictive controller returns a joint
...
\subsection{Dual quaternion kinematics}
The end-effector pose is a unit dual quaternion
$\underline{\bm q}_{ee}=\bm q_r+\varepsilon\,\bm q_d$, with
$\bm q_d=\tfrac12\,\bm t\otimes\bm q_r$. The error against a desired
$\underline{\bm q}_d$ is
$\underline{\bm q}_e=\underline{\bm q}_d^{*}\otimes\underline{\bm q}_{ee}$ and its
logarithm is
\begin{equation}
\bm e=\log\underline{\bm q}_e
=\begin{bmatrix}\bm\varphi\\ \bm\rho\end{bmatrix}\in\sethree,
\qquad
\bm\rho=\bm J_\ell(\bm\varphi)^{-1}\bm t_{\mathrm{err}},
\label{eq:logdual}
\end{equation}
in which the translational part is expressed in the frame of the rotational error
through the inverse left Jacobian of $\mathrm{SO}(3)$~\cite{sola2018microlie}.
This coupling distinguishes \eqref{eq:logdual} from a decoupled error and its
contribution is quantified in Section~\ref{sec:res-ablation}.
...
\subsection{Admittance in $\sethree$}
...
dual quaternion reference directly, so translation and rotation remain one object.

\subsection{Dual quaternion predictive controller}
...
The logarithm \eqref{eq:logdual} differs from a decoupled error only through
...
manifold dominate the compromise. We therefore do not advance the dual quaternion
formulation as a performance claim. Its merit is that it supplies the
