...
\begin{proposition}[Directional structure of a yaw-plus-coplanar-pitch arm]
Let joint $1$ rotate about the vertical $\bm e_z$ and joints $2$ to $4$ about a
common horizontal axis $\bm a$, and let $\mathcal{P}=\Jv(\ker\Jw)$. If
...
and the numerical error of $\hat{\bm f}_{3}$ is at machine precision. That
statement involves no metric and no length. Second, over the same set the
six-dimensional inversion discards $50.5\,\%$ of the applied force on average
under the unweighted metric of standard practice, $38.0\,\%$ under the arm's own
characteristic length, and in the worst configuration and direction essentially
all of it under either. Table~\ref{tab:metric} gives the full dependence; only
