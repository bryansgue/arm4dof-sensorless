...
must be built from motor current. External wrenches have been reconstructed from
internal signals alone for fine manipulation~\cite{shan2024fine} and with fast
...
has been demonstrated on a surgical assistant~\cite{zhang2021forcefree}. Closest
to the actuation regime considered here, de Wolde \emph{et al.} adapt impedance
control to current-controlled arms without force or torque sensors, calibrating
the actuator current-to-torque ratios and friction~\cite{dewolde2024current}. That
work commands current, so the actuator remains a torque interface; the arm
considered here exposes a velocity command and closes its own inner loop, which is
what makes the model assignment of Section~\ref{sec:velocity} necessary. On the
...
is known, and has been exploited prescriptively: Lu \emph{et al.} optimize the
configurations of a six-joint industrial robot using the Jacobian condition number
as the indicator, reducing the effect of modeling error without reducing the error
itself~\cite{lu2023configopt}. Two things differ here. The bound
\eqref{eq:noise} is stated on the point-contact inverse rather than on the
six-dimensional one, so it applies in the regime where the six-dimensional
inverse is not merely ill-conditioned but indeterminate; and
$\sigma_{\min}(\Jv)$ is used as a gate that declines to report, rather than as an
objective to maximize, because on a four-joint arm the posture is often dictated
by the task and not available as a free variable. The bound is verified as a law on
the simulated plant in Section~\ref{sec:res-cond}.
