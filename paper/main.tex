...
with the least observable eigendirection, exactly so within each posture and at
correlation $0.9993$ over all $12\,000$ directions pooled. The analysis separates
what is a property of the arm from what is a property of an undeclared unit
convention. The minimum-norm criterion sums newtons and newton-meters, so it
presupposes a length; we parameterize it, show that the least observable direction
and the set of exactly recovered forces are invariant to that length while the
magnitude of the loss is not, and report the sensitivity. The workspace sweep then
supplies magnitude and prevalence, $50.5\,\%$ of an applied unit force discarded
on average over $2197$ configurations under the unweighted convention that
standard practice implements, $38.0\,\%$ under the arm's own characteristic
length, and up to $99.6\,\%$ under either; $\sigma_{\min}(\Jv)$ converts the
analysis into an operating criterion, with its cost in retained workspace stated;
and the correlation study shows why an apparently sound validation can conceal a
large gain error, at $0.997$ against a $2.227$~N error. Against a second,
independently written dynamic implementation the corrected inversion reduces force
...
Several ingredients of this framework are individually established, and it is
worth listing them so that the contribution is not read as larger than it is. That
...
stated in~\cite{magrini2014virtual}; that a posture can leave a task-space
direction unobserved by joint torque sensing, and that this can be measured by a
per-configuration index, is established
in~\cite{wong2024sensorobs}; that Jacobian conditioning governs the accuracy of
the reconstruction and can be used to select postures is established
in~\cite{lu2023configopt}; motor current has been used in place of joint torque
sensing~\cite{wahrburg2018motorcurrent}, and the point-contact reduction is already
the practical default on low-cost arms~\cite{yen2019virtual}. What is contributed
is the quantity none of them reports, namely how much of an applied force the
six-dimensional inversion discards, in which directions, and how often across a
workspace; the observation that this quantity is not scale-free, and the
separation of the part that is; the demonstration that the loss is silent under a
conventional validation; and the velocity-actuated regime, which that literature
does not address.

The $50.5\,\%$ figure is not itself a general result. It belongs to this arm, this
workspace, this distribution of push directions, and one particular choice of the
length that renders the wrench norm homogeneous. It carries weight because $\Pff$
explains and predicts it, because its directional structure is invariant to that
choice, and because the same construction can be evaluated for any other arm
before one is built.
...
here rather than headlined. What does not move is the worst case, $99.6\,\%$ for
